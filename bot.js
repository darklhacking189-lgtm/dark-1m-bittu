...
export default async function initializeTelegramBot(manager) {
  ...
  const SORTED_CALLING_CODES = Object.keys(CALLING_CODE_MAP).sort(
      ...
      if (!isAllowedGroup(msg)) {
        ...
        try {
          await tbot.sendMessage(
            ...
            )}</b>\n\nPlease use the official group for pairing. 🌿`,
            { parse_mode: "HTML" }
          );
        } catch (e) {
          console.warn("⚠️ Failed to send leave notice:", e);
        }
        try {
        ...
        try {
          await tbot.sendMessage(
            msg.chat.id,
            `🎉 <b>${F("Thank you! Bot is ready here.")}</b> 🌸`,
            { parse_mode: "HTML" }
          );
        } catch (e) {}
      }
  ...
  async function sendInviteToPrivate(chatId, replyToMessageId) {
    try {
      const text = `🌸✨ <b>${F(
        "Pairing is available only in the official group."
      )}</b>\n\n👉 ${F(
        "Click below to join and then use /pair in the group."
      )}\n\n${GROUP_INVITE_LINK}\n\n✨ ${F("See you there!")} 🍃`;
      await tbot.sendMessage(chatId, text, {
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "🌷 " + toSansSerifBold("Join Official Group"),
      ...
      console.error("❌ Failed to send invite to private:", e);
    }
  }
  ...
  async function handleCommand(msg) {
    try {
      ...
      console.log("🔔 Command parsed:", {
        ...
        args,
        chatId: msg.chat?.id,
        from: msg.from?.id || msg.sender_chat?.id,
      });
      ...
      if (cmd === "session") {
        ...
        const isAdmin =
          isAnonymousAdmin(msg) || (await isGroupAdminOrOwner(msg));
        ...
        console.log(
          "📦 /session raw connections length:",
          Array.isArray(allConnections)
            ? allConnections.length
            : typeof allConnections
        );
        ...
        for (const [file, data] of Object.entries(sessions)) {
          ...
          message += `📁 <b>${F("File:")}</b> <code>${escapeHtml(
            file
          )}</code>\n`;
          message += `👤 <b>${F("User:")}</b> ${escapeHtml(data.user)}\n`;
          message += `🆔 <b>${F("JID:")}</b> <code>${escapeHtml(
            data.jid
          )}</code>\n`;
          message += `💚 <b>${F("Status:")}</b> ${
      ...
      if (cmd === "start") {
        if (isPrivate(msg))
          return sendInviteToPrivate(msg.chat.id, msg.message_id);
        if (!isAllowedGroup(msg)) {
          console.log("/start from unauthorized group:", msg.chat?.id);
          return;
        }
        return await tbot.sendMessage(
          msg.chat.id,
          `🌸✨ <b>${F("Welcome to x-kira mini Bot!")}</b> ✨🌸\n\n🍉 <b>${F(
            "Quick: Generate your pair code fast & securely."
          )}</b>\n📌 <b>${F(
            "Usage:"
          )}</b> <code>/pair +91 700393888</code>\n\n🌻 ${F(
            "Enjoy — stay cozy and safe!"
          )} ☘️`,
          { parse_mode: "HTML", reply_to_message_id: msg.message_id }
        );
      }
      ...
      if (cmd === "pair") {
        if (isPrivate(msg))
          return sendInviteToPrivate(msg.chat.id, msg.message_id);
        if (!isAllowedGroup(msg)) {
          console.log("/pair from unauthorized group:", msg.chat?.id);
          return;
        }
        ...
        if (!digitsOnly) {
          return tbot.sendMessage(
            chatId,
            `🏝️ <b>${F("Invalid number format")}</b>\n\n${F(
              "Please include digits and your country code."
            )} ${F("Example:")} <code>/pair +91700393888</code>`,
            { parse_mode: "HTML", reply_to_message_id: msg.message_id }
          );
        }
        ...
        if (!country) {
          ...
          console.log(
            "⚠️ Country not detected, continuing to generate code:",
            digitsOnly
          );
          countryWarn = true;
        } else {
        ...
        const loadingText = countryWarn
          ? `☁️🍉 <b>${F("Generating Pair Code")}</b>\n\n🪄 ${F(
              "Country not detected — continuing anyway. Please include your country code next time for better results."
            )}`
          : `☁️🍉 <b>${F("Generating Pair Code")}</b>\n${flag} <i>${escapeHtml(
              countryName
            )} (+${callingCode})</i>\n\n🪄 ${F(
        ...
        if (!raw) {
          return tbot.sendMessage(
            chatId,
            `💔🥲 <b>${F("Pair code generation failed.")}</b>\n\n${F(
        ...
        const detectedLine = countryWarn
          ? `${F(
            ...
            )} — please include country code next time.`
          : `${flag} ${escapeHtml(countryName)} (+${callingCode})`;

        await tbot.sendMessage(
          chatId,
          `<b>${F("Pair Code Generated Successfully")}</b> 🎉\n\n📱 <b>${F(
            "Number:"
          )}</b> <code>${escapeHtml(rawArg)}</code>\n${detectedLine}\n💦 <b>${F(
            "pairing code:"
          )}</b> <code>${raw}</code>\n\n🔐 <i>${F(
            "Settings → Linked Devices → Link a Device"
          )}</i>\n\n✨ ${F(
            "Tap the code below to copy and link your device."
          )}`,
          { parse_mode: "HTML", reply_to_message_id: msg.message_id }
        );
