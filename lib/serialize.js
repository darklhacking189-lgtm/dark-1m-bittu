...
export default class Serializer {
  ...
  serializeSync(msg) {
    try {
      ...
      const pushName = msg.pushName || msg.pushname || "Unknown";
      ...
      // determine if message is from this bot user
      const botUserId = jidNormalizedUser(this.conn?.user?.id || "");
      const botLid = this.conn?.user?.lid
        ? jidNormalizedUser(this.conn.user.lid)
        : null;
      const isFromMe =
        fromMe ||
        areJidsSameUser(sender, botUserId) ||
      ...
      // --- group helpers (lazy) ---
      msgObj.getParticipants = () => {
        const list = msgObj.groupParticipants || [];
        // participants could be array of strings or objects with id
        return list.map((p) => (typeof p === "string" ? p : p?.id || p));
      };
      msgObj.isParticipant = (jid) => {
        const normalized = jidNormalizedUser(jid);
        return (msgObj.getParticipants() || []).some((p) =>
          areJidsSameUser(jidNormalizedUser(p), normalized)
        );
      };
      ...
      msgObj.loadGroupInfo = async () => {
        ...
        try {
          ...
          msgObj.groupAdmins = (msgObj.groupParticipants || [])
            ...
            .map((p) => jidNormalizedUser(p.id || p));
      ...
      msgObj.download = async () => {
        try {
          ...
          return await this._downloadMediaFromMsg(
            msg,
            ct.replace("Message", "")
          );
        } catch (err) {
      ...
      msgObj.send = async (payload, options = {}) => {
        try {
          ...
          if (options.quoted) {
            const quotedObj =
              options.quoted?.raw ?? options.quoted ?? msgObj.raw;
      ...
      msgObj.profilePictureUrl = async (jid, type = "image") => {
        try {
          return await this.conn.profilePictureUrl(
      ...
      // never throw from the sync serializer — return null so caller can fallback
      console.error("serializeSync fatal error:", err);
      return null;
    }
  }
